...
ggplot(total_cancellation_orders_and_items_per_month, aes(x = InvoiceDate, y = Total_Orders, group = 1)) + 
  geom_line(color = '#52854C') + geom_point(color = '#52854C') +
...
ggplot(total_cancellation_orders_and_items_per_month, aes(x = InvoiceDate, y = Total_Units, group = 1)) +
  geom_line(color = 'darkred') + geom_point(color = 'darkred') +
...
ggplot(revenue_per_month, aes(x = InvoiceDate, y = Total_Revenue, group = 1 )) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + geom_area(fill = '#E7B800')

# c. Orders Made In The Last 120 Days
...
last_120_days <- ymd(max(orders2$InvoiceDate)) - 120

orders_last_120_days <- orders2 %>% filter(InvoiceDate > last_120_days) %>% 
